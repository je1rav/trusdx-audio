...
def handle_cat(pastream, ser, cat):
    if(cat.inWaiting()):
        ...
        if True and d.startswith(b'ID'):   # this is a workaround for unrealistic fast RTT expectations in hamlib for sequence RX;ID;
            ...
            return
        if status[0]:
            # ---from here; answer cat polling without sending to ADX during TX
            if d.startswith(b'MD'):
                cat.write(b'MD2;')
                log(f"I: {d}")
                log(f"O: MD2; (emu)")
                return
            if d.startswith(b'FW'):
                cat.write(b'FW0000;')
                log(f"I: {d}")
                log(f"O: FW0000; (emu)")
                return
            if d.startswith(b'FA'):
                send = "FA" + freq + ";"
                cat.write(send.encode('utf-8'))
                log(f"I: {d}")
                log(f"O: " + send + " (emu)")
                return   
            if d.startswith(b'IF'):
                send = "IF" + freq + "00000+000000000120000000;"
                cat.write(send.encode('utf-8'))
                log(f"I: {d}")
                log(f"O: " + send + " (emu)")
                return    
